...
CasualSchema.pre('save', function (next) {
...
CasualSchema.statics.REQUEST = async function() 
	{
		...
		if (callback instanceof Function) 
		{
			...
			try 
			{
				while (null !== (casual = await cursor.next())) 
				{
					if (asynch) 
					{
						try 
						{
							await callback(casual);
						} catch (err) { throw err; }
					}
					else
						{
